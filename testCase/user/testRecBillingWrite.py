...
recBilling_xls = ExcelReader().get_xls("RecBilling.xlsx", "recBillingWrite")


@paramunittest.parametrized(*recBilling_xls)
class RecBillingWrite(unittest.TestCase):
    def setParameters(self, case_name, action, merchantnumber, org_recbillnumber, recbillname, recbillinfo_name,
                      ...
                      cellphone_1, cellphone_2, cellphone_3, memo, transcode, writeoffmemo, rc, rm):
        """
        set params
        ...
        self.writeoffmemo = str(writeoffmemo)
        self.rc = str(rc)
        self.rm = str(rm)
    ...
    def testRecBillingWrite(self):
        ...
        data = {
            'data': {'action': self.action,
                     ...
                     'cellphone_2': self.cellphone_2,
                     'cellphone_3': self.cellphone_3,
                     'memo': self.memo,
            ...
            if (self.info['rc'] == '0000'):
                ...
                data = {
                    'data': {'action': 'writeoff',
                             'merchantnumber': self.merchantnumber,
                             'org_recbillnumber': self.org_recbillnumber.format(timestamp=self.timestamp),
                             'writeoffmemo': self.writeoffmemo
                             }
                }
                self.return_json = apiTester.send_rec_billing_writeoff_command(data, self.transcode)
                Common.show_return_msg(self.return_json)
